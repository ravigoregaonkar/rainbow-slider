...
(function () {
    ...
    // Debounce resize to avoid canvas reset on every pixel change (especially during iPad rotation)
    ...
    function update() {
        ...
        // Check D coin collection (use squared distance - avoids Math.sqrt per frame)
        const coinDistThreshSq = ((player.width + coinSize) / 2.5) ** 2;
        coins.forEach((coin) => {
            if (!coin.collected) {
    ...
    function createFirework(x, y) {
        // Cap total particles to prevent frame drops on iPad
        if (fireworks.length >= MAX_FIREWORK_PARTICLES) return;

        const colors = ['#FFD700', '#FF3B30', '#00E5FF', '#76FF03', '#FF00FF', '#FFFFFF'];
        const count = Math.min(25, MAX_FIREWORK_PARTICLES - fireworks.length);
    ...
    function drawFireworks() {
        // Draw all particles in one pass without save/restore per particle
        fireworks.forEach(f => {
    ...
    function draw() {
        ...
        // Draw Super Coins (shadow removed - too expensive on iPad GPU)
        superCoins.forEach((sc) => {
            if (!sc.collected && !sc.missed) {
                ctx.save();
                ...
                // Sparkle every 40 frames instead of 20 to halve overdraw
                if (sc.sparkleTimer % 40 === 0) {
                    ctx.save();
                    ctx.globalAlpha = 0.7;
                    ctx.fillStyle = '#FFD700';
        ...
        if (GAME.superMode) {
            ctx.save();
            // Fixed shadow blur - no sine per frame (too expensive on mobile GPU)
            ctx.shadowColor = '#FFD700';
            ctx.shadowBlur = 25 * Math.max(GAME.superModeAnimation, 0.2);
        }
    ...
    // ===== DELTA-TIME GAME LOOP (fixes speed on 120Hz iPad & slow laptops) =====
    const TARGET_FPS = 60;
    // const FIXED_STEP = 1000 / TARGET_FPS;
    const FIXED_STEP = 1000 / 48;
    let accumulator = 0;
    let lastFrameTime = 0;
    ...
    function gameLoop(timestamp) {
        ...
        // Only update logic at fixed 60fps steps, draw once
